...
def display_population_with_input(population):
    ...
    for i, p in enumerate(st.session_state.population):
        ...
        with cols[i % 2]:
            ...
            selected_label = st.selectbox("配色の順番を変更", pattern_labels, key=f"order_{st.session_state.gen}_{i}")
            ...
            dynamic_svg = f"""
            <svg id="_レイヤー_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 875.13 877.13" style="width:100%; max-width:200px; height:auto; margin:10px 0;">
                <defs>
                    <clipPath id="clippath">
                        <rect x="0" y="0" width="284.99" height="493.61" style="fill:none;"/>
                    </clipPath>
                    <pattern id="_헥사곤_패턴" x="0" y="0" width="284.99" height="493.61" patternTransform="translate(1525.5352 1551.4831) scale(1.2738)" patternUnits="userSpaceOnUse" viewBox="0 0 284.99 493.61">
                ...
                <rect y="1" width="875.13" height="875.13" style="fill:url(#_헥사곤_패턴);"/>
            </svg>"""
            ...
            # 2. value引数を削除し、値の保持は key に完全にお任せする
            st.number_input(
                f"評価 (0〜10)", 0, 10, 
                key=state_key
            )
...
if st.session_state.is_finished:
    ...
    bc = st.session_state.selected_gene
    bc_hex = hsv_to_rgb_hex(bc['h'], bc['s'], bc['v'])
    
    fs = st.session_state.final_selection
    f_hexes = [hsv_to_rgb_hex(c['h'], c['s'], c['v']) for c in (fs['base_color_data'], fs['gene1_color_data'], fs['gene2_color_data'])]
    f_ids = [c['ID'] for c in (fs['base_color_data'], fs['gene1_color_data'], fs['gene2_color_data'])]
    ...
    <body style="font-family: sans-serif; background-color: #1e1e1e; color: #ffffff; padding: 20px;">
        ...
        <h3 style="color: #ffffff;">🎨 最終決定した配色</h3>
        <div style="display: flex; gap: 10px; align-items: center;">
            <div style="width:50px; height:50px; background-color:{f_hexes[0]}; border:1px solid #555;"></div>
            <div style="width:50px; height:50px; background-color:{f_hexes[1]}; border:1px solid #555;"></div>
            <div style="width:50px; height:50px; background-color:{f_hexes[2]}; border:1px solid #555;"></div>
            <span style="margin-left: 10px; font-size: 16px;">({', '.join(f_ids)})</span>
        </div>

        <h3 style="color: #ffffff;">🤖 LLMの解釈（ターゲットデータ）</h3>
        <pre style="background-color: #2b2b2b; color: #ffffff; padding: 10px; border-radius: 5px; border: 1px solid #444;">{json.dumps(st.session_state.target_data_list, indent=2, ensure_ascii=False)}</pre>
    ...
    for gen_data in st.session_state.history:
        ...
        for idx, ind in enumerate(gen_data['population']):
            h1 = hsv_to_rgb_hex(ind['base_color_data']['h'], ind['base_color_data']['s'], ind['base_color_data']['v'])
            h2 = hsv_to_rgb_hex(ind['gene1_color_data']['h'], ind['gene1_color_data']['s'], ind['gene1_color_data']['v'])
            h3 = hsv_to_rgb_hex(ind['gene2_color_data']['h'], ind['gene2_color_data']['s'], ind['gene2_color_data']['v'])
            
            user_score_display = ind.get('user_score', '-')
            
            report_html += f"""
            <div style='border:1px solid #555; background-color: #2b2b2b; padding:8px; font-size:12px; margin-bottom: 5px; color: #ffffff; border-radius: 5px;'>
                <div style='margin-bottom: 5px;'>No.{idx+1} (Sys:{ind.get('final_score',0):.1f} | User:{user_score_display})</div>
                <div style='display:flex;'>
                    <div style='width:20px; height:20px; background-color:{h1};'></div>
                    <div style='width:20px; height:20px; background-color:{h2};'></div>
                    <div style='width:20px; height:20px; background-color:{h3};'></div>
                </div>
            </div>
            """
